document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    const render = (nIdx) => {
      ...
      if (cortesElementos[idx]) cortesElementos[idx].classList.remove('activo');
    ...
    // Asignación segura de acciones
    const asignarAccion = (id, accion) => {
      const el = $(id);
      if (!el) return;
      el.onclick = (e) => { e.preventDefault(); accion(); };
      el.ontouchstart = (e) => { e.preventDefault(); e.stopPropagation(); accion(); };
    };
    ...
    console.error("Error en visor PACS:", error);
  } finally {
    // SE EJECUTA SI O SI: Elimina la pantalla de carga incluso ante fallos graves
    const loader = document.getElementById('tc-loading');
